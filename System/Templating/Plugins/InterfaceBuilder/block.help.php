...
function smarty_block_help($params, $content, &$smarty, &$repeat){
    ...
    if(isset($params['buttonize']) && !SmartestStringHelper::toRealBool($params['buttonize'])){
        ...
        $html = '<a href="#'.SmartestStringHelper::toSlug('help-'.$params['id']).'" class="sm-help-link" id="'.$id.'"><span>'.$content.'</span></a><script type="text/javascript">$(\''.$id.'\').observe(\'click\', function(e){HELP.load(\''.$params['id'].'\');Event.stop(e);})</script>';
    }
