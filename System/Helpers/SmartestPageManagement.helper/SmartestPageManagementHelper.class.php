...
class SmartestPageManagementHelper extends SmartestHelper{
	...
	public function getPageChildren($parent_page_id, $site_id, $draft_mode=false, $for_web=false){
		
		$parent_page = new SmartestPage;
	...
	public function getPagesTree($site_id, $startPageId='', $level=0){
		...
		if(SmartestCache::hasData('site_pages_tree_'.$site_id, true)){
			...
			$pageTree[0]["info"] = $this->getPageAsArray($startPageId);
	...
	public function getPagesSubTree($startPageId, $site_id, $level=0){
		...
		foreach($_children as $child){
			...
			$workArray[$index]["children"] = $this->getPagesSubTree($child["id"], $site_id, $level+1);
			$index++;
	...
	function getOkParentPages($page_id){
		...
		if($page->hydrate($page_id)){
    		...
    		$all_pages = $this->getSerialisedPageTree($this->getPagesTree($page->getParentSite()->getId()));
    		$this->displayPages = array();
    		$this->displayPagesIndex = 0;
		
    		// THEN GET A LIST OF ALL CHILD PAGES
    		$sub_pages = $this->getSerialisedPageTree($this->getPagesSubTree($page_id, $page->getParentSite()->getId()));
    		$this->displayPages = array();
    		$this->displayPagesIndex = 0;
    		...
    		foreach($sub_pages as $page_array){
    			if(! in_array($page_array["info"]["id"], $sub_page_ids)){
    				$sub_page_ids[] = $page_array["info"]["id"];
    			}
    		}
		
    		// REMOVE THOSE PAGES FROM THE MAIN LIST
    		foreach($all_pages as $key=>$page_array){
    			if(in_array($page_array["info"]["id"], $sub_page_ids) || $page_array["info"]["id"] == $page_id){
    				unset($all_pages[$key]);
    			}
    		}
	...
	public function getPageAsArray($page_id){
