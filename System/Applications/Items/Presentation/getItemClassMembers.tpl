...
{foreach from=$items key="key" item="item"}
    <li ondblclick="window.location='{$domain}{$section}/openItem?item_id={$item.id}'" class="item {if $item.public=='FALSE'}unpublished{else}published{/if} {if $item.is_archived=='1'}archived{else}current{/if}">
      <a href="#" data-approved="{makebool value=$item.changes_approved assign="approved"}{$approved.truefalse}" data-archived="{makebool value=$item.is_archived assign="archived"}{$archived.truefalse}" data-created="{$item.created.unix}" data-published="{makebool value=$item.public assign="published"}{$published.truefalse}" class="option" id="item_{$item.id}" onclick="return itemList.setSelectedItem('{$item.id}', 'item', {literal}{{/literal}updateFields: {literal}{{/literal}item_name_field: '{$item.name|summary:"29"|trim|_item_name_escape}'{literal}}{/literal}{literal}}{/literal});">
