...
{if $property.is_image_property && $sm_user_agent.is_supported_browser}
  ...
  {else}
  {image_select id=$input_id name=$_input_data.name value=$value for="ipv" property=$property}
  {/if}
    ...
    <ul class="item_property_actions">
      ...
      {if isset($value.type_info) && is_array($value.type_info)}
      ...
      {if $item && $item.id && isset($value.type_info) && is_array($value.type_info) && isset($value.type_info.param) && count($value.type_info.param)}
...
<div id="itemproperty-asset-select-{$property.id}" style="display:{if (!$value || !$value.id) && ($property.foreign_key_filter == 'SM_ASSETTYPE_RICH_TEXT' || $property.foreign_key_filter == 'SM_ASSETCLASS_TEXT')}none{else}block{/if}">
{asset_select id=$input_id name=$_input_data.name value=$value options=$property._options required=$property.required}

{if (!$value || !$value.id) && ($property.foreign_key_filter == 'SM_ASSETTYPE_RICH_TEXT' || $property.foreign_key_filter == 'SM_ASSETCLASS_TEXT')} <a href="#cancel" class="button small" id="cancel-asset-text-select-{$property.id}">Cancel</a>{/if}
</div>

<script type="text/javascript" src="{$domain}Resources/System/Javascript/tinymce8/tinymce.min.js"></script>
<script type="text/javascript" src="{$domain}Resources/System/Javascript/smartest/tinymce8-smartest.js"></script>

  {if (!$value || !$value.id) && ($property.foreign_key_filter == 'SM_ASSETTYPE_RICH_TEXT' || $property.foreign_key_filter == 'SM_ASSETCLASS_TEXT')}
  <div class="edit-form-sub-row" style="margin-bottom:10px" id="asset-text-buttons-{$property.id}">
    <div style="margin-bottom:5px">{if $item && $item.id}This {$item.model.name|lower} has no text at present{else}This is a new item which does not yet have any text for this property.{/if}</div>
    {if $item && $item.id}
  ...
  <script type="text/javascript">
  {if !$item || !$item.id}$('new-asset-button-{$property.id}').observe('click', function(e){ldelim}
    e.stop();
  ...
  <ul class="item_property_actions">
    ...
    {if $value && $value.id && $item && $item.id && $value.is_editable}
    ...
    {elseif $value && $value.id && $item && $item.id && is_array($value.type_info) && ((!isset($value.type_info.editable) || !_b($value.type_info.editable)) && isset($value.type_info.param) && count($value.type_info.param))}
    ...
    {if $value && $value.id && $item && $item.id && is_array($value.type_info) && isset($value.type_info.param) && count($value.type_info.param)}
