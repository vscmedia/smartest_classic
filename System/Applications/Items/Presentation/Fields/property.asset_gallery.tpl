...
{asset_group_select id=$property_id name=$name value=$value options=$property._options required=$property.required}
  ...
  <ul class="item_property_actions">
    ...
    <li style="display:{if is_numeric($value.id)}block{else}none{/if}" id="reorder-gallery-property-{$property.id}"><a href="{$domain}assets/arrangeAssetGallery?from=item_edit&amp;group_id={$value.id}&amp;from=editItem&amp;item_id={$item.id}{if $request_parameters.page_id}&amp;page_id={$request_parameters.page_id}{/if}" title="Arrange this gallery" id="edit-gallery-button-{$property.id}"><i class="fa fa-random"></i></a></li>
