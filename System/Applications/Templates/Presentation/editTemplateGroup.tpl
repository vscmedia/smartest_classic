<div id="work-area">
  ...
  <h3>Edit template group <span class="light">"{$group.label}"</span></h3>
  ...
  <form action="{$domain}{$section}/updateTemplateGroup" method="post">
    ...
    <div>
      ...
      <div class="edit-form-row">
        ...
        {if $allow_name_edit}<input type="text" name="group_name" value="{$group.name}" /><div class="form-hint">letters, numbers and underscores only, please.</div>{else}{$group.name}{/if}
      </div>
