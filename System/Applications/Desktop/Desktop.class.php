...
class Desktop extends SmartestSystemApplication{
	...
	public function buildSite($get, $post){
	    ...
	    $p->setParameter('site_domain', $this->getRequestParameter('site_domain'));
		    ...
		    try{
		        $site = $sch->createNewSite($p, $this->getUser(), $build_kit_name, $prepared_buildkit_params);
    ...
    public function aboutSmartest(){
        ...
        // Code management
        if(is_dir(SM_ROOT_DIR.'.git/')){
            $code_management = "GIT";
            // TODO: access Git commit ID and send
        }elseif(is_dir(SM_ROOT_DIR.'.svn/')){
            $code_management = "SVN";
        }else{
            $code_management = "None";
        }
        $this->send($code_management, 'code_management');

        // Show Facebook Box?
        // $this->send($this->getGlobalPreference('hide_facebook_like_box', 0), 'hide_fb_likes_box');
        $this->send(1, 'hide_fb_likes_box');
